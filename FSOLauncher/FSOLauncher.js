...
class FSOLauncher extends Events {
    ...
    constructor(Window, conf) {
        ...
        this.Window.on('minimize', () => {
            if (!this.minimizeReminder) {
                Modal.sendNotification('FreeSO Launcher', global.locale.MINIMIZE_REMINDER)
     ...
     */
    runFullInstaller() {
        (new(require('./Library/Installers/CompleteInstaller'))(this))
        .run()
    }
    ...
    removeActiveTask(Name) {
        if (Name) {
            return this.ActiveTasks.splice(
                this.ActiveTasks.indexOf(Name), 1
            )
        }
    ...
    async editTTSMode(value) {
        ...
        try {
            ...
            fs.writeFile(this.isInstalled.FSO + '\\Content\\config.ini', ini.stringify(data), err => {
                this.removeActiveTask('CHTTS')
                Toast.destroy()
    ...
    async checkLauncherUpdates(wasAutomatic) {
        if (!this.isSearchingForUpdates &&
            ...
            let Toast = new ToastComponent(global.locale.TOAST_CHECKING_UPDATES, this.View)
            ...
            try {
                ...
                if (data.Version !== global.version) {
                    if (this.lastUpdateNotification !== data.Version && !this.Window.isVisible()) {
    ...
    install(Component, options) {
        ...
        switch (Component) {
            ...
            case 'NET':
                ...
                return new Promise(async (resolve, reject) => {
                    let file = Component === 'NET' ? 'NDP46-KB3045560-Web.exe' : 'oalinst.exe'
    ...
    async switchLanguage(language) {
        ...
        fs.remove(this.isInstalled.FSO + '\\Content\\Patch\\User\\translations', async (error) => {
            fs.copy(path.join(__dirname, '../export/LanguagePacks/' + language.toUpperCase() + '/TSO'), this.isInstalled.TSO + '\\TSOClient', async (error) => {
                ...
                fs.copy(path.join(__dirname, '../export/LanguagePacks/' + language.toUpperCase() + '/FSO'), this.isInstalled.FSO, async (error) => {
                    if (error) {
                        this.removeActiveTask('CHLANG')
                        Toast.destroy()
                    ...
                    try {
                        ...
                        fs.writeFile(this.isInstalled.FSO + '\\Content\\config.ini',
                            ini.stringify(data), (error) => {
                                this.removeActiveTask('CHLANG')
                                Toast.destroy()

                                if (error) return Modal.showIniFail()

                                this.conf.Game.Language =
    ...
    enableSoftwareMode() {
        ...
        return new Promise((resolve, reject) => {
            fs.copy('bin/dxtn.dll', this.isInstalled.FSO + '\\dxtn.dll', async (error) => {
                if(error) {
                    Toast.destroy()
                    reject(error)
                    return this.removeActiveTask('CHSWM')
                ...
                fs.copy('bin/opengl32.dll', this.isInstalled.FSO + '\\opengl32.dll', async (error) => {
                    if(error) {
                        Toast.destroy()
                        reject(error)
                        return this.removeActiveTask('CHSWM')
        ...
        fs.stat(this.isInstalled.FSO + '\\FreeSO.exe', (err, stat) => {
            if (err === null) {
                ...
                fs.stat(this.isInstalled.FSO + '\\FreeSO.exe.old', (err, stat) => {
                    if (err === null) {
                        fs.rename(this.isInstalled.FSO + '\\FreeSO.exe.old', this.isInstalled.FSO + '\\FreeSO.exe', (err) => {
    ...
    launchGame(useVolcanic) {
        ...
        require("child_process").exec(
            ...
            (this.conf.Game.GraphicsMode!='sw'?this.conf.Game.GraphicsMode:'ogl') + ((this.conf.Game['3DMode'] === '1'&&this.conf.Game['GraphicsMode']!='sw') ? ' -3d' : ''), {
    ...
    createShortcut(path) {
        return new Promise((resolve, reject) => {
            ...
            ws.create('%UserProfile%\\Desktop\\FreeSO.lnk', {
    ...
    getFSOConfig() {
        return new Promise((resolve, reject) => {
            ...
            fs.readFile(this.isInstalled.FSO + '\\Content\\config.ini', 'utf8', (err, data) => {
    ...
    persist(showToast) {
        ...
        require('fs').writeFile(
            'FSOLauncher.ini', require('ini').stringify(this.conf),
            (err) => {
                setTimeout(() => {
                    Toast.destroy()
                }, 1500);
