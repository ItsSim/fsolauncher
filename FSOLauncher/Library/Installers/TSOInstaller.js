...
class TSOInstaller {
    ...
    error(ErrorMessage) {
        ...
        this.createProgressItem(
            global.locale.TSO_FAILED_INSTALLATION,
            100
        );
    ...
    download() {
        return new Promise((resolve, reject) => {
            this.dl.start((failedCabs) => {
                return failedCabs === 0 ?
                    resolve() : reject(global.locale.NETWORK_ERR_DL_X + ' ' + failedCabs + ' ' + global.locale.X_FILES_OCURRED)
            }).then(() => {
            ...
            if (p.percentage < 100) {
                if (!this.haltProgress) {
                    this.createProgressItem(
                        ...
                        d + " " + global.locale.X_FILES_OUT_OF_X + " 1114 (" + p.mbDownloaded + " MB, " + p.percentage + "%)",
                        p.percentage
                    );
                ...
                errFile => {
                    if (errFile) {
                        return require('fs').unlink(errFile, () => {
                            reject('Extraction failed. ' + errFile + ' was corrupted and has been deleted. Try running the installer again.');
                        })
                    }
                    resolve();
                }
