...
var Electron = require("electron"),
    ipcRenderer = Electron.ipcRenderer, rss = require("rss-parser"),
    cloudscraper = require("cloudscraper"),
    shell = Electron.shell, FSOLauncher = function () {
...
FSOLauncher.prototype.registerUIEvent = function (a, b, c) {
    a.tagName ? a.addEventListener(b, c) : document.querySelector(a).addEventListener(b, c)
};
FSOLauncher.prototype.loadRss = function () {
    cloudscraper.get(document.querySelector('body').getAttribute('rss'), (err, response, body) => {
        rss.parseString(body, function (a, b) {
            ...
            a ? document.querySelector("#rss .alt-content").style.display = "block" : (document.querySelector("#rss .alt-content").style.display = "none", document.querySelector("#rss-root").innerHTML = "", b.feed.entries.forEach(function (a) {
                var b = document.createElement("div"), c = document.createElement("h1"), g = document.createElement("span"),
                    d = document.createElement("div"), h = document.createElement("a");
                ...
                d.innerHTML = ct;
                h.innerHTML = document.querySelector('body').getAttribute('rmstr');
                h.className = "button";
...
FSOLauncher.prototype.registerUIEventAll = function (a, b, c) {
    ...
    for (a = 0; a < e.length; a++)e[a].addEventListener(b, function (a) {
        c(a, e)
    })
};
FSOLauncher.prototype.showHints = function (a) {
    for (var b = document.querySelectorAll("[hint-page]"), c = 0; c < b.length; c++)b[c].style.display = "none";
    if (!localStorage["HINT_" + a]) {
        b = document.querySelectorAll('[hint-page="' + a + '"]');
        for (c = 0; c < b.length; c++)b[c].style.display = "block", b[c].addEventListener("click", function (a) {
...
FSOLauncher.prototype.changePage = function (a) {
    for (var z = document.querySelectorAll('li[page-trigger]'), x = 0; x < z.length; x++) {
        z[x].className = z[x].className.replace("active", "");
    }

    document.querySelector("li[page-trigger='"+ a +"']").className += " active";

    for (var b = document.querySelectorAll("div.page"), c = b.length - 1; 0 <= c; c--)b[c].style.display = "none";
    document.querySelector("#" + a + "-page").style.display = "block";
    this.showHints(a)
};
FSOLauncher.prototype.restoreConfiguration = function (a) {
    for (Section in a)for (Item in a[Section]) {
        var b = document.querySelector('[option-id="' + Section + "." + Item + '"');
...
FSOLauncher.prototype.updateFullInstallProgressItem = function (a, b, c, e) {
    ...
    if (a && b && c) {
        var g = document.querySelector("#full-install-title"), d = document.querySelector("#full-install-text1"),
            h = document.querySelector("#full-install-text2"), k = document.querySelector("#full-install-progress");
        g.innerHTML = a;
        d.innerHTML = b;
        h.innerHTML = c;
        k.style.width = e + "%";
...
FSOLauncher.prototype.createNotification = function(title, body, url) {
    ...
    var contentContainer = document.querySelector('#notifications-page .page-content');
        contentContainer.innerHTML = notification.outerHTML + contentContainer.innerHTML;
        
    document.querySelector('#FSONotification'+id + ' p').addEventListener('click', function (e) {
...
FSOLauncher.prototype.createOrModifyProgressItem = function (a, b, c, e, f, g) {
    ...
    if (d) d.querySelector("h1").innerHTML = b, d.querySelector("span").innerHTML = c, d.querySelector(".info").innerHTML = e, d.querySelector(".progress").style.width = f + "%", g ? (d.querySelector(".loading").style.display = "none", f = d.querySelector(".miniconsole"), f.innerHTML += g, f.style.display = "block", f.scrollTop = f.scrollHeight) : (d.querySelector(".loading").style.display =
        "block", d.querySelector(".miniconsole").style.display = "none"); else {
        ...
        g ? (e.style.display = "none", h.innerHTML += g, h.style.display = "block", h.scrollTop = h.scrollHeight) : (e.style.display = "block", h.style.display =
            "none");
        ...
        d.appendChild(h);
        d.appendChild(e);
        document.querySelector("#downloads-page .page-content").innerHTML = d.outerHTML + document.querySelector("#downloads-page .page-content").innerHTML
    }
};
FSOLauncher.prototype.yesNo = function (a, b, c, e, f, g) {
    ...
    b.appendChild(a);
    e ? (c = document.createElement("span"), c.innerHTML = e, c.addEventListener("click",
        function () {
            d.parentNode.removeChild(d);
            if(document.querySelectorAll('.modal').length == 0) {
                document.querySelector("#overlay").style.display = "none";
        ...
        }), b.appendChild(c)) : a.style.margin = "0px";
    d.appendChild(b);
...
FSOLauncher.registerServerEvent("CREATE_PROGRESS_ITEM", function (a, b, c, e, f, g, d) {
    FSOLauncher.createOrModifyProgressItem(b, c, e, f, g, d)
});
FSOLauncher.registerServerEvent("FULL_INSTALL_PROGRESS_ITEM", function (a, b, c, e, f) {
...
FSOLauncher.registerUIEventAll("[option-id]", "change", function (a, b) {
    var c = a.currentTarget.getAttribute("option-id"), e = a.currentTarget.value;
