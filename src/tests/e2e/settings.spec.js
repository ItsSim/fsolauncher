...
test( 'changes the theme', async () => {
  await window.click( '[page-trigger="settings"]' );
  const themeSelector = await window.waitForSelector( '[option-id="Launcher.Theme"]' );
  await themeSelector.selectOption( 'indigo' );

  expect( await ( await window.$( 'body' ) ).getAttribute( 'class' ) ).toContain( 'indigo' );
} );